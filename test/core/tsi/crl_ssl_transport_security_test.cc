  ...
  class SslTsiTestFixture {
   public:
    static SslTsiTestFixture* Create(bool use_revoked_server_cert,
      ...
      new (fixture) SslTsiTestFixture(use_revoked_server_cert,
   ...
   private:
    ...
    void SetupHandshakers() {
      ...
      EXPECT_EQ(tsi_create_ssl_client_handshaker_factory_with_options(
                     &client_options, &client_handshaker_factory_), TSI_OK);
    ...
    void CheckHandshakerPeers() {
      ...
      // handshake succeeds.
      bool expect_server_success = !(use_revoked_server_cert_ ||
                                     use_revoked_client_cert_);
      ...
      if (expect_client_success) {
        EXPECT_EQ(tsi_handshaker_result_extract_peer(
                      base_.client_result, &peer), TSI_OK);
      ...
      if (expect_server_success) {
        EXPECT_EQ(tsi_handshaker_result_extract_peer(
                       base_.server_result, &peer), TSI_OK);
        tsi_peer_destruct(&peer);
      } else {
