  ...
  class ServerThread {
   public:
    explicit ServerThread(XdsEnd2endTest* test_obj)
        : test_obj_(test_obj), port_(grpc_pick_unused_port_or_die()) {}
    ...
    void Serve(grpc_core::Mutex* mu, grpc_core::CondVar* cond) {
      ...
      if (test_obj_->use_xds_enabled_server_) {
    ...
    bool use_xds_enabled_server() const {
      return test_obj_->use_xds_enabled_server_;
    }
...
TEST_P(BasicTest, InitiallyEmptyServerlist) {
  ...
  std::thread delayed_resource_setter(std::bind(
      &BasicTest::SetEdsResourceWithDelay, this, balancer_.get(),
...
TEST_P(XdsResolverOnlyTest, CircuitBreakingMultipleChannelsShareCallCounter) {
  ...
  SetNextResolutionForLbChannelAllBalancers();
  SetNextResolutionForLbChannelAllBalancers(nullptr,
                                            lb_response_generator2.get());
...
TEST_P(ClientLoadReportingTest, Vanilla) {
  if (GetParam().use_fake_resolver()) {
    balancer_->lrs_service()->set_cluster_names({kServerName});
  }
  SetNextResolution({});
  SetNextResolutionForLbChannelAllBalancers();
  const size_t kNumRpcsPerAddress = 10;
