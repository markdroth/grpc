  ...
  for (size_t priority = 0; priority < priority_list.size(); ++priority) {
      ...
      if (!child_number.has_value()) {
        ...
        if (it != locality_child_map.end()) {
          ...
          for (XdsLocalityName* old_locality :
            ...
            gpr_log(GPR_INFO,
                    "      REMOVING OTHER LOCALITIES FROM THIS CHILD FROM "
                    "PREVIOUS UPDATES: %s",
                    old_locality->AsHumanReadableString().c_str());
            locality_child_map.erase(old_locality);
          }
        }
      } else {
        gpr_log(GPR_INFO,
                "    CHILD ALREADY FOUND, REMOVING NEW LOCALITY FROM MAP");
    ...
    if (!child_number.has_value()) {
      ...
      for (child_number = 0;
           ...
           ++(*child_number)) {
        gpr_log(GPR_INFO, "    CHILD NUMBER %" PRIuPTR " FOUND IN MAP",
                *child_number);
      }
