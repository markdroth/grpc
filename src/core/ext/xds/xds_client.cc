...
void XdsClient::ChannelState::LrsCallState::MaybeStartReportingLocked() {
  gpr_log(GPR_INFO, "==> MaybeStartReportingLocked(): reporter_=%p",
          reporter_.get());
  // Don't start again if already started.
  if (reporter_ != nullptr) return;
  gpr_log(GPR_INFO, "  call_=%p send_message_pending_=%d", call_.get(),
  ...
  if (chand()->ads_calld_ != nullptr) {
    gpr_log(GPR_INFO, "  chand()->ads_calld_->calld()=%p",
            chand()->ads_calld_->calld());
    if (chand()->ads_calld_->calld() != nullptr) {
      gpr_log(GPR_INFO, "  chand()->ads_calld_->calld()->seen_response()=%d",
...
RefCountedPtr<XdsClient::ChannelState> XdsClient::GetOrCreateChannelStateLocked(
  ...
  gpr_log(GPR_INFO,
          "==> GetOrCreateChannelStateLocked(%s (%p), %s): channel map:",
          server.server_uri().c_str(), &server, reason);
  for (const auto& p : xds_server_channel_map_) {
    gpr_log(GPR_INFO, "  %s (%p) => %p", p.first->server_uri().c_str(), p.first,
            p.second);
  }
